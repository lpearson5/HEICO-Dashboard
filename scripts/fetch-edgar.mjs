 ...
 * Run from a home/personal network — corporate networks trigger SEC bot detection.
 */
...
async function batch(items, fn, concurrency = CONCURRENCY) {
  ...
  for (let i = 0; i < items.length; i += concurrency) {
    ...
    if (i > 0 && i % 100 === 0) console.log(`    … ${i}/${items.length} processed`);
  }
  return out;
}
...
async function getXmlUrl(filer) {
  ...
  if (indexRes) {
    ...
    if (candidates.length) {
      return candidates.find(x => /infotable|informationtable|13finfo/i.test(x)) ?? candidates[candidates.length - 1];
    }
  }
...
async function processFiler(filer, cusips) {
  try {
    ...
    // Skip if SEC served a block/error page instead of real XML
...
async function main() {
  ...
  console.log("Step 1: Getting filer lists...");
  ...
  console.log(`\nStep 2: Scanning ${total} filings for HEICO positions...`);
